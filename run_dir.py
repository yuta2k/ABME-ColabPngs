...
@torch.no_grad()
def doInterpolate(srcFirst, srcSecond, dst):
    frame1 = TF.to_tensor(imread(srcFirst)).unsqueeze(0).cuda()
    frame3 = TF.to_tensor(imread(srcSecond)).unsqueeze(0).cuda()
...
for i, imgFile in enumerate(imgFiles):
    ...
    ipDstPath = os.path.join(args.dst, f'{args.dst_filename}' % (outIndex + 1))
    outIndex += 2

    if os.path.exists(ipDstPath):
        continue

    if not args.no_copy:
        shutil.copyfile(currSrcPath, cpDstPath)
        print('[ABME] \'%s\' copied!'%cpDstPath)

    if i + 1 < len(imgFiles):
        nextSrcPath = os.path.join(args.src, imgFiles[i + 1])
        doInterpolate(currSrcPath, nextSrcPath, ipDstPath)
